  ...
  const columns: ColumnDef<Event>[] = useMemo(
    () => [
        ...
        : []),
      {
        ...
        cell: ({ row }) => (
          <span
            className={cn("rounded-full px-2 py-1 font-semibold text-xs", {
    ...
    return Object.entries(numericFieldSums).map(([field, sums]) => {
      ...
      return (
        <Card key={field}>
          ...
          <div>
            <p className="font-bold text-2xl">{relevantSum.toFixed(2)}</p>
        ...
        <TabsContent value={activeTab}>
          <div className="mb-14 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card className="border-2 border-brand-700">
              ...
              <div>
                <p className="font-bold text-2xl">{data?.eventsCount || 0}</p>
                <p className="text-muted-foreground text-xs/5">
                  Events{" "}
                  {activeTab === "today"
                    ? "today"
